...
- **Services order is deliberate: Audit 01, AI 02, Brand 03, Campaign 04, Content 05** (reordered 2026-09-03). AI moved up from fifth because it is the growth product against evidenced sector demand; it is not first because the page's own logic depends twice on Communications audit being the entry point. The five services appear in the same order in SIX places and they move together: `services.html` blocks and numbering, the anchor bar, the JSON-LD `@graph`, the `index.html` stack-pill list, `llms.txt:8`, and the `contact.html` select with its `SERVICE_LABELS` twin in `api/contact.js`.
